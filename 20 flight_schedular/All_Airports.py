...
class AllAirports:
    ...
    def load_airport_data(self, file_path):
        ...
        # dictionary country <--> currancy name
        with open('./data/countrycurrency.csv', 'r') as file:
            lines = csv.reader(file)
            for line in lines:
                country_currancy[line[0]] = line[1]
        file.close()
        ...
        with open(file_path, 'r', encoding='utf8')as file:
            ...
            try:
                for line in lines:
                    country = line[3]
                    currency = country_currancy[country]
    ...
    def distance_detween_two_airports(self, airport1_code, airport2_code):
        airport1 = self.airports[airport1_code]
        airport2 = self.airports[airport2_code]
        distance = self.get_distance_between_two_airports(airport1.lat, airport1.lon, airport2.lat, airport2.lat)
        return distance

    def get_ticket_price(self, start, end):
        distance = self.distance_detween_two_airports(start, end)
